...
=======
			...
			if tasks && errors.empty?
		      @tasks.save
					...
					if @notify.include?(', ')
						@notify=@notify.split(', ')
					elsif @notify.include?(',')
						@notify=@notify.split(',')
					end
					
					@to_user=@notify
					@to_user<<params[:task][:recipient] if params[:task][:recipient]
...
>>>>>>> 2ce3e5e8b6e1935b249cd3cc768d63964352ffdb:app/controllers/tasks_controller.rb
  end
