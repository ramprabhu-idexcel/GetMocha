class ProjectMailer < ActionMailer::Base
  default :from => "mochabot@getmocha.com", :except=>['message_notification','message_reply']
    def project_renamed(user,old_project,new_project,to_user)
      ...
      mail(:to=>"#{to_user.email}", :subject=>"#{old_project} Project Has Been Renamed to #{new_project}",:content_type=>"multipart/alternative")
    end
  ...
  def custom_email(user, invite)
    ...
    @invite="#{APP_CONFIG[:site_url]}/projects/verify_email/#{invite.verification_code}"
     mail(:to=>"#{user.email}", :subject=>"Verify Your Email Address on Mocha",:content_type=>"multipart/alternative")
    @content_type="text/html"
  end
  def message_notification(user,to_user,message)
    ...
    mail(:from=>"#{from}", :to=>"#{to_user}", :reply_to=>"ctzm#{message.id}@#{APP_CONFIG[:reply_email]}", :subject=>"#{user.first_name} posted a new message to #{to_user}",:content_type=>"multipart/alternative")
    @content_type="text/html"
  end
  def invite_people(user,invite)
    ...
    mail(:to=>"#{invite.email}", :subject=>"#{user.full_name} has invited you to join #{invite.project.name} on GetMocha.com",:content_type=>"multipart/alternative")
    @content_type="text/html"
  end
  def message_reply(user,comment)
    ...
    mail(:from=>"#{from}",  :to=>@user.email,:reply_to=>"ctzm#{@message.id}@#{APP_CONFIG[:reply_email]}", :subject=>@message.subject,:content_type=>"multipart/alternative")
